...
function captureScreenshot(emotion, faceBox, allDetections) {
    const now = Date.now();
...
function updatePersonDetails(person) {
    ...
    if (person) {
        ...
        personPhoto.src = 'placeholder-avatar.png';
        personName.textContent = 'Person Name';
            ...
            if (now - lastEmotionUpdate >= EMOTION_UPDATE_INTERVAL) {
                ...
                detections.forEach((detection, index) => {
                    ...
                    if (negativeEmotions.includes(emotion) && confidence > EMOTION_THRESHOLD) {
                        ...
                        captureScreenshot(emotion, detection.detection.box, detections);
